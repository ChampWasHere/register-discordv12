...
module.exports.run = async (client, message, args) => {
    ...
    if (!üye) return message.lineReply('`Kayıt edebilmem için bir üye belirtmelisin!`').then(x => x.delete({ timeout: 2000 }));
    ...
    if (!üye.user.username.includes('ᵃᶻᵗ') && !üye.user.discriminator.includes('0086')) return message.lineReply('`Bu üyede tagımız bulunmuyor!`').then(x => x.delete({ timeout: 3000 }));

    await üye.setNickname(`${id.tag} ${isim} - ${yaş}`), üye.roles.add(id.kızrol), üye.roles.remove(id.kayıtsızrol);
    ...
    message.lineReply(new Discord.MessageEmbed().setColor('GREEN').setDescription(`${üye} üyesine başarılı bir şekilde <@&${id.kızrol}> rolünü verip sunucuya kaydettim. \n **Kullanıcının önceki kayıtları**;\n ${ismleri}`)).then(x => x.delete({ timeout: 10000 }), message.react(id.başarılıemojiid));
